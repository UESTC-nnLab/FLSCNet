...
class TrainSetLoader(Dataset):
    ...
    def __init__(self, dataset_dir, img_id, base_size=512, crop_size=480, transform=None, suffix='.png'):
        ...
        self.dataset = dataset_dir.split('/')[-1]
        self.base_size = base_size
        self.crop_size = crop_size
        self.suffix = suffix

    def _sync_transform(self, img, mask, dataset):
    ...
    def __getitem__(self, idx):
        ...
        img, mask = self._sync_transform(img, mask, self.dataset)
...
def save_result(dt_string, epoch, train_loss, test_loss, m_IoU, recall, precision, save_mIoU_dir,
                save_other_metric_dir):
    with open(save_mIoU_dir, 'a') as f:
        f.write('{} epoch:{:04d}\t - train_loss: {:04f}:\t - test_loss: {:04f}:\t mIoU {:.4f}\n'.format(
            dt_string, epoch, train_loss, test_loss, m_IoU))
...
def save_model(mean_IoU, best_IoU, save_dir, save_prefix, train_loss, test_loss, recall, precision, epoch, net,
               optimizer, scheduler):
    save_mIoU_dir = 'result/' + save_dir + '/' + save_prefix + '_epoch_IoU.log'
    save_other_metric_dir = 'result/' + save_dir + '/' + save_prefix + '_epoch_other_metric.log'
    save_best_IoU_dir = 'result/' + save_dir + '/' + save_prefix + '_best_IoU_IoU.log'
    save_best_IoU_other_metric_dir = 'result/' + save_dir + '/' + save_prefix + '_best_IoU_other_metric.log'
    now = datetime.now()
    dt_string = now.strftime('%d/%m/%Y %H:%M:%S')
    save_result(dt_string, epoch, train_loss, test_loss, mean_IoU,
    ...
    if mean_IoU > best_IoU:
        save_ckpt({
            ...
            filename='Best_IoU_' + '_' + save_prefix + '_epoch' + '.pth.tar')
        save_result(dt_string, epoch, train_loss, test_loss, mean_IoU,
                    recall, precision, save_best_IoU_dir, save_best_IoU_other_metric_dir)
...
def make_dir(dataset, model):
    ...
    save_dir = '%s_%s_wDS' % (dataset, model)
    os.makedirs('result/%s' % save_dir, exist_ok=True)
    return save_dir


def total_visualization_generation(dataset_dir, test_txt, suffix, target_image_path, target_dir):
...
def save_Pred_GT(pred, labels, target_image_path, val_img_ids, index, suffix):
    ...
    img.save(target_image_path + '/' + '%s_Pred' % (val_img_ids[index]) + suffix)
    img = Image.fromarray(labelsss.reshape(256, 256))
    img.save(target_image_path + '/' + '%s_GT' % (val_img_ids[index]) + suffix)


def save_Pred_GT_visualize(pred, img_demo_dir, img_demo_index, suffix):
    predsss = np.array((pred > 0).cpu()).astype('int64') * 255
    predsss = np.uint8(predsss)
...
def save_and_visualize_demo(pred, labels, target_image_path, val_img_ids, num, suffix):
    predsss = np.array((pred > 0).cpu()).astype('int64') * 255
    predsss = np.uint8(predsss)
    labelsss = labels * 255
    labelsss = np.uint8(labelsss.cpu())
